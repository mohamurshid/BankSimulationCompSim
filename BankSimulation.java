...
class OutputWindow extends JFrame {
    ...
    // ── TABLE PANEL ───────────────────────────
    private JPanel buildTablePanel(SimulationResult r) {
        String[] cols = {
                "#", "Inter-Arrival (min)", "Arrival Time", "Service Time (min)",
                "Service Start", "Wait Time (min)", "Service End", "Time in System (min)"
        };

        Object[][] data = new Object[r.n][cols.length];
        for (int i = 0; i < r.n; i++) {
            data[i][0] = i + 1;
            data[i][1] = DF.format(r.interArrival[i]);
            data[i][2] = DF.format(r.arrivalTime[i]);
            data[i][3] = DF.format(r.serviceTime[i]);
            data[i][4] = DF.format(r.serviceStart[i]);
            data[i][5] = DF.format(r.waitTime[i]);
            data[i][6] = DF.format(r.serviceEnd[i]);
            data[i][7] = DF.format(r.timeInSystem[i]);
        }
